...
async function aiSummary() {
  ...
  LLMData.messages[1].content = '后面是一个 JSON 数组，每个数组成员是一个对象，包括 title 和 link 两个字段。你将 title 字段翻译成中文，并抓取 link 字段的网址，生成该网址的中文内容总结，每个总结长度不要超过300字，总结内部的双引号要替换成 HTML 实体（即 &quot;），再将总结放在双引号中，添加到该对象的 summary 字段。最后，以 JSON 格式返回所有内容，不要返回其他内容，也不要放在 json 代码块里面，并在返回前检查这个 JSON 格式的字符串，确保它能被 JavaScript 语言的 JSON.parse() 正确解析。 ' + JSON.stringify(linkArray);
